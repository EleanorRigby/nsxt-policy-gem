...
module SwaggerClient
  ...
  class Condition
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'member_type')
    ...
    def list_invalid_properties
      ...
      if @member_type.nil?
        invalid_properties.push('invalid value for "member_type", member_type cannot be nil.')
      end
    ...
    def valid?
      ...
      operator_validator = EnumAttributeValidator.new('String', ['EQUALS', 'CONTAINS', 'STARTSWITH', 'ENDSWITH', 'NOTEQUALS'])
      return false unless operator_validator.valid?(@operator)
      return false if @value.nil?
      return false if @value.to_s.length < 1
      return false if @key.nil?
      key_validator = EnumAttributeValidator.new('String', ['Tag', 'Name', 'OSName', 'ComputerName'])
      return false unless key_validator.valid?(@key)
      return false if @member_type.nil?
      member_type_validator = EnumAttributeValidator.new('String', ['IPSet', 'VirtualMachine', 'LogicalPort', 'LogicalSwitch'])
    ...
    def key=(key)
      validator = EnumAttributeValidator.new('String', ['Tag', 'Name', 'OSName', 'ComputerName'])
    ...
    def member_type=(member_type)
      validator = EnumAttributeValidator.new('String', ['IPSet', 'VirtualMachine', 'LogicalPort', 'LogicalSwitch'])
    ...
    def hash
      [_self, _links, _schema, _revision, _system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, path, parent_path, relative_path, children, marked_for_delete, operator, value, key, member_type].hash
    end
