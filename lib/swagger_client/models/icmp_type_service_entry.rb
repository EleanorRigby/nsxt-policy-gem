...
module SwaggerClient
  ...
  class ICMPTypeServiceEntry
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'icmp_type')
    ...
    def list_invalid_properties
      ...
      if !@icmp_code.nil? && @icmp_code > 255
        invalid_properties.push('invalid value for "icmp_code", must be smaller than or equal to 255.')
      end
      ...
      if !@icmp_type.nil? && @icmp_type > 255
        invalid_properties.push('invalid value for "icmp_type", must be smaller than or equal to 255.')
      end
    ...
    def valid?
      ...
      return false if !@icmp_code.nil? && @icmp_code > 255
      return false if !@icmp_code.nil? && @icmp_code < 0
      return false if !@icmp_type.nil? && @icmp_type > 255
      return false if !@icmp_type.nil? && @icmp_type < 0
      return false if @protocol.nil?
      protocol_validator = EnumAttributeValidator.new('String', ['ICMPv4', 'ICMPv6'])
    ...
    def icmp_code=(icmp_code)
      if !icmp_code.nil? && icmp_code > 255
        fail ArgumentError, 'invalid value for "icmp_code", must be smaller than or equal to 255.'
      end
    ...
    def icmp_type=(icmp_type)
      if !icmp_type.nil? && icmp_type > 255
        fail ArgumentError, 'invalid value for "icmp_type", must be smaller than or equal to 255.'
      end
    ...
    def protocol=(protocol)
      validator = EnumAttributeValidator.new('String', ['ICMPv4', 'ICMPv6'])
    ...
    def hash
      [_self, _links, _schema, _revision, _system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, path, parent_path, relative_path, children, marked_for_delete, icmp_code, icmp_type, protocol].hash
    end
