...
module SwaggerClient
  # Filter parameters for filtering routes from RIB depending on provided properties. 
  class LogicalRouterRIBRequestParameters
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'route_source')
        self.route_source = attributes[:'route_source']
      end
    end
    ...
    def valid?
      ...
      route_source_validator = EnumAttributeValidator.new('String', ['BGP', 'STATIC', 'CONNECTED'])
      return false unless route_source_validator.valid?(@route_source)
      true
    end
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] route_source Object to be assigned
    def route_source=(route_source)
      validator = EnumAttributeValidator.new('String', ['BGP', 'STATIC', 'CONNECTED'])
      unless validator.valid?(route_source)
        fail ArgumentError, 'invalid value for "route_source", must be one of #{validator.allowable_values}.'
      end
      @route_source = route_source
    end
    ...
    def hash
      [source, transport_node_id, network_prefix, route_source].hash
    end
